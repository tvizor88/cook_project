...
// MongoDB connection
mongoose.connect('mongodb://localhost:27017/recipes', { useNewUrlParser: true, useUnifiedTopology: true }); // connect DB
...
const { title, section, description, steps, time, coverImage, stepImages } = req.body;

// Отладочные сообщения
console.log('Request body:', req.body);
console.log('Files:', req.files);
console.log('Cover Image:', req.files ? req.files['coverImage'] : 'No cover image');
console.log('Step Images:', req.files ? req.files['stepImages[]'] : 'No step images');

// Сохранение рецепта в базе данных
const recipe = new Recipe({ title, section, description, steps, time, coverImage, stepImages });
await recipe.save();
